...
msg_err_range: .string "Error: Immediate out of range."
msg_err_addr: .string "Error: Invalid address format."
...
.word op_auipc, 2
.word op_addi, 3
.word op_slli, 7      # 5-bit shift
.word op_srli, 7      # 5-bit shift
.word op_srai, 7      # 5-bit shift
.word op_andi, 3
.word op_ori, 3
.word op_xori, 3
.word op_slti, 3
...
# Check if token is a valid register (x0-x31 or ABI names)
check_is_register:
    mv t0, a0
    lb t1, 0(t0)
...
skip_ws_addr:
    lb t1, 0(t0)
    beqz t1, addr_op2_fail
    li t2, 32
    beq t1, t2, skip_ws_addr_inc
    li t2, 44
    beq t1, t2, skip_ws_addr_inc
    j build_addr
skip_ws_addr_inc:
